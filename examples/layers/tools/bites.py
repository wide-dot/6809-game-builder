"""Detect red bites in a video, track them across frames, print runs."""
import sys
    ...
    for cx, cy, a in comps:
        ...
        for t in tracks:
            ...
            if d < BD and d < 24:
                ...
                BEST = t
        if BEST is None:
            BEST = {'id': NID, 'maxarea': a, 'low': 0}
            NID += 1
        else:
            tracks.remove(BEST)
        BEST.update(cx=cx, cy=cy, area=a)
        BEST['maxarea'] = max(BEST['maxarea'], a)
        if a < 0.75 * BEST['maxarea']:
            BEST['low'] += 1
        else:
            if BEST['low'] >= 3:
                BITES.append((FRAMES, BEST['id'], BEST['low'],
                              BEST['maxarea'], int(cx), int(cy)))
            BEST['low'] = 0
        new.append(BEST)
    tracks = new
    FRAMES += 1
for t in tracks:
    if t['low'] >= 3:
        BITES.append((FRAMES, t['id'], t['low'], t['maxarea']))
print(path, 'frames', FRAMES, 'comps median', int(np.median(COMPS_HIST)),
      'min', min(COMPS_HIST), 'max', max(COMPS_HIST))
