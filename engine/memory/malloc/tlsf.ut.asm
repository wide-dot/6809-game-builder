...
!       clr   tlsf.err
        ...
        beq   >
        bra   *
        ; the 4 byte block lands in the cell of size 4 : (fl=3, sl=4) with
        ; 16 classes, fl capped at its minimum and sl = the size itself ;
        ; (fl=2, sl=0) with 4 classes, no cap (bsr(4)=2 > min fl 1)
 IFEQ tlsf.SL_BITS-4
!       ldd   tlsf.fl.bitmap
        ...
        bne   *
        ldd   tlsf.headMatrix+(tlsf.SL_SIZE*3+4)*2
        cmpd  #$1111
        bne   *
 ENDC
 IFEQ tlsf.SL_BITS-2
!       ldd   tlsf.fl.bitmap
        cmpd  #%0000000000000100       ; fl=2
        bne   *
        ldd   tlsf.sl.bitmaps+2*2
        cmpd  #%0000000000000001       ; sl=0
        bne   *
        ldd   tlsf.headMatrix+(tlsf.SL_SIZE*2+0)*2
        cmpd  #$1111
        bne   *
 ENDC
...
!       ldd   tlsf.fl.bitmap
        ...
        cmpd  #%0000000000000001       ; sl=0
        bne   *
        ldd   tlsf.headMatrix+(tlsf.SL_SIZE*15)*2
        cmpd  #$2222
        bne   *
...
tlsf.ut.mappingSearch.out
        ; the (fl, sl) each input maps to, after the round up to the next
        ; class : one table per supported tlsf.SL_BITS (the 4 class one was
        ; generated by a model of tlsf.mappingSearch that reproduces the 16
        ; class table exactly ; the unit test is what validates the model)
 IFEQ tlsf.SL_BITS-4
        fcb   3,1
        fcb   3,2
        fcb   3,3
 ...
 IFNE tlsf.SL_BITS-4
 IFNE tlsf.SL_BITS-2
        ERROR tlsf.ut only knows the expectations of tlsf.SL_BITS 4 and 2
 ENDC
 ENDC
...
tlsf.ut.realloc.doOneTest
        ...
        bra   * ; error trap
!
        ; the biggest request the free block ($3EF7 bytes left after the $101
        ; block) still serves : a request is rounded up to the next size
        ; class before the search, so it must stay in the class of that block
        ; — 16 classes : $3DFF (+$1FF rounds to $3FFE, class (13,15) with the
        ; block) ; 4 classes : $3800 (+$7FF rounds to $3FFF, class (13,3)) ;
        ; $3DFF at 4 classes rounds past $4000 and is a (correct) OUT_OF_MEMORY
 IFEQ tlsf.SL_BITS-4
        ldd   #$3DFF
 ENDC
 IFEQ tlsf.SL_BITS-2
...
!
        ...
        bra   * ; error trap
!
        ; allocate near full memory : $2EEF are left ; the request must map
        ; into that block's class after its round up (see above) — 16
        ; classes : $2DFF (+$1FF = $2FFE, class (13,7) with the block) ;
        ; 4 classes : $2800 (+$7FF = $2FFF, class (13,1) with the block).
        ; Whatever is left behind it does not matter to the three realloc
        ; cases below : u1 merges with the u0 hole only, 257+4+4096 = $1105.
 IFEQ tlsf.SL_BITS-4
        ldd   #$2DFF
 ENDC
 IFEQ tlsf.SL_BITS-2
