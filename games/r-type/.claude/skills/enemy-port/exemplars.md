...
| Mort d'un boss (stages 2-8 : gomander, bellmite, bydo…) | `common/fx/bosscascade/obj.asm` + `gomander/cascade.unit.asm` + `tools/gen_gomander_death.py` | LE patron arcade de la cascade (a6a5/b4c9/c284) : un enfant commun qui égrène une table {dx, dy, subtype} GÉNÉRÉE depuis la ROM et échantillonnée à 8 images/s (un spawn arcade sur quatre, la grosse explosion pré-tirée), sa propre unité/direntry, le slot commun 29 (`ObjID_bosscascade`) ; le boss lève un octet de sa page (`gomander.dying`) que ses enfants lisent pour se retirer comme sous a523 |
