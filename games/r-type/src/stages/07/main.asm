;*******************************************************************************
; Stage 7 — mis en place avec sa carte (chantier stages 3-8)
;
...
; tire huit par salve (neuf p-staff dans la wave de ce stage) ; sans ces deux
; entrees son index tombait sur le bouchon et les roquettes naissaient
; inertes. Meme correctif qu'au stage 4 le 29/08.
...
stage.music.page  equ stage7.music.ymm.page
...
scantfire.Object  EXTERNAL
 ...
 SECTION code
        ...
        INCLUDE "gen/stages/07/map/map.const.asm"
...
; checkpoint ($0B00,ObjID_checkpoint dans wave.asm) et sa position est son
; horodatage divise par 128 — 128 trames par tuile, l'inverse exact de la
; vitesse de scroll (cf. src/common/flow/checkpoint.unit.asm). La table de
; reference v1 (src/common/state/checkpoint/checkpoint-data.asm) concorde.
...
stage.setup
        ...
        ldd   #map.odd
        std   scroll_map_odd
        lda   #map.RAM_OVER_CART+stage7.maps.page
        ...
        lda   #map.RAM_OVER_CART+stage7.wave.page
...
; Fin du stage 7 : on enchaîne sur le stage 8. La campagne remonte
...
;*******************************************************************************
        INCLUDE "src/stages/07/objid.const.asm"
        INCLUDE "src/stages/07/objid.index.asm"
