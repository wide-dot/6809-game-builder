...
slither.RPP       equ 12                 ; le potentiel du tour precedent : une
                                         ; BAISSE est un coup encaisse, et vaut
                                         ; une trame blanche
...
@aloop  ldb   slither.mActive,u
        ...
        sta   slither.RPP,x            ; la reference du flash de coup
...
@img
        ; --- LE FLASH DE COUP -----------------------------------------------
        ; Le potentiel d'un record n'est plus rearme depuis que la collision
        ; est systematique : il accumule, donc une BAISSE est un coup encaisse.
        ; L'etat part dans le SLOT (1 normal, 2 blanc) — c'est lui qui repartit
        ; le travail entre les deux renderers, chacun ne montant qu'une page.
        lda   #1
        sta   slither.wState
        ldb   slither.wN
        jsr   slither.BoxPtrB
        lda   AABB.p,x
        sta   slither.wPot
        beq   @noflash                 ; a zero : le verdict de mort suffira
        ldb   slither.wN
        jsr   slither.RecPtrB
        lda   slither.wPot
        cmpa  slither.RPP,x
        ...
        sta   slither.wState
@noflash
        ; --- le set d'images : corps normal, ou corps blanc ------------------
        ldb   slither.wPose
        andb  #$0F
        aslb
...
slither.RecPublish
        pshs  a,b
        _sprite.cull slither.RecPublish.off              ; le containment, comme le moteur
        lda   ,s
        suba  6,x
        sta   1,y
        lda   1,s
...
@vivant
        ...
        bhs   @noflash
        sta   slither.fPrevP,u
        _soundFX.play soundFX.HitSound,0     ; borne $57 (tete, corps et queue)
        lda   #1
