;*******************************************************************************
; cancer — ennemi porté de la v1 (stage 1)
;
...
; Les properties v1 : 0..2 = cancer_*.png tel quel, 3..5 = les mêmes en miroir
; X (l'ennemi vient de la gauche ou de la droite).
Img_cancer_0                 equ set_cancer_0
Img_cancer_1                 equ set_cancer_1
Img_cancer_2                 equ set_cancer_2
Img_cancer_3                 equ set_cancer_3
Img_cancer_4                 equ set_cancer_4
Img_cancer_5                 equ set_cancer_5

; V2-DEVIATION : l'entrée v1 s'appelle `Object`, un nom trop générique pour la
; frontière de lien.
;
; Les deux scripts d'animation des properties v1 (Ani_cancerleft /
; Ani_cancerright) ne sont PAS portés : le fichier v1 ne les cite jamais — il
; anime par sa propre table d'images, celle qui suit PresetXYIndex. On ne porte
; que ce que le code demande.
cancer.Object
        INCLUDE "src/enemies/cancer/obj.asm"

; Le temoin de rattrapage : pose a la naissance d'un cancer, relu par
; RunCancerDisplacement pour sortir apres le deplacement, efface aussitot. La
; v1 le range dans la RAM du stage (ram_data.asm) ; il vit ici parce qu'il est
; TRANSITOIRE et que cancer est le seul a le connaitre — pose et efface dans
; la meme naissance, il n'a rien a survivre. L'unite est chargee en RAM, donc
; il s'ecrit ; sa page est montee, puisque le code qui l'ecrit y tourne.
cancerCatchup
        fcb   0
