#!/usr/bin/env python3
"""Le cout du gouger au compteur de cycles, par appel — pas au profileur.

    python3 tools/gouger_cost_probe.py dist/to8.fd
...
for _ in range(80):
    ...
    if gougers().get(1, 0) >= 1:
        break
print('gougers', gougers(), flush=True)
measure(1, 'Hidden', 4)
for _ in range(60):
    t.call('run_frames', {'n': 25, 'fast': True, 'timeout_ms': 600000})
    if gougers().get(2, 0) >= 1:
        break
print('gougers', gougers(), flush=True)
measure(2, 'Dive', 6)
# measure_probe(4) : plus de sonde de decor depuis les profils precalcules
t.close()
