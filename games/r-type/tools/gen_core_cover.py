...
est dans une cavite de la coque, et sa glissade de 13 px le fait passer sous
la masse de coque de droite, dont seule la cavite (pen 0) le laisse voir.
...
Ce script decoupe la fenetre dans src/stages/03/map/battleship.png autour de
la position de REPOS du noyau (mesuree sous toje : x = 352, y = 102 en
coordonnees de couche), et rend transparent le noir de la cavite — les
...
REST = (352, 102)                  # l'ancre de repos du noyau, en couche (toje)
WIN = (340, 84, 388, 120)          # la fenetre : cavite + glissade + coque
...
def main():
    ...
    w, h = crop.size
    px = crop.load()
    # le noir de la cavite : flood fill depuis le centre du noyau, sur l'index 1
    seed = (REST[0] - x0, REST[1] - y0)
    assert px[seed] == NOIR, 'le centre du noyau n\'est pas sur du noir : %d' % px[seed]
    pile = [seed]
    vus = set()
    while pile:
        x, y = pile.pop()
