#!/usr/bin/env python3
"""Capture video+son des stages 1 ET 2 d'affilee, en mode INVINCIBLE.

    TOJE_MCP=<toje>/scripts/toje-mcp.sh TOJE_FAST=1 \
    python3 tools/stages12_video.py dist/to8.fd [dist/stages12.avi]
...
DIFFERENCE avec stage1_video.py : la capture ne s'arrete pas au premier
changement de stage. Le stage 1 rend la main au stage 2 (bench.stage passe de
1 a 2) et le film continue ; on ne coupe qu'en sortant des DEUX, c'est-a-dire
quand bench.stage n'est plus ni 1 ni 2. Le rechargement du stage 2 passe par
le loader, qui garde la page DATA — rien a faire de special, la capture est
cote emulateur et ne depend pas de ce que le jeu pagine.
"""
import os, re, sys
...
out = os.path.abspath(sys.argv[2] if len(sys.argv) > 2 else 'dist/stages12.avi')
...
BUDGET = int(os.environ.get('STAGE_FRAMES', '60000'))
done = 0
while done < BUDGET:
    ...
    if st not in (1, 2):
        print('les stages 1 et 2 sont passes (stage=%d) — 100 trames de queue'
              % st, flush=True)
