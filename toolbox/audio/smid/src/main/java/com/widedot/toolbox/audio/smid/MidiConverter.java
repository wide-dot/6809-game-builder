...
public class MidiConverter{
	...
	private boolean remap_gm2mt32 = false;
	
	public MidiConverter(File file, String remap) throws Exception {
		
        log.info("Process {}", file.toString());
		
		remap_gm2mt32 = remap.equals("MT");
		...
		Files.write(new File(FileUtil.removeExtension(file.toString())+".smid").toPath(), data);	
		
        log.info("Successfully processed {}", file.toString());
	}
	
	public List<long[]> initTempo(Sequence sequence, int tickpQrt) {
	...
	public byte[] getMT32Data(Track track, int tickpQrt, List<long[]> tempoLst) throws Exception {
		...
		for (int i = 0; i < track.size(); i++) {
			...
			// keep a selection of basic messages, may evolve if needed
			MidiMessage message = event.getMessage();
			if (!(message instanceof MetaMessage) && !(message instanceof SysexMessage) && (
				...
				(!remap_gm2mt32 && message instanceof SysexMessage)) {
				...
				while (relativeFrame > 0) {
					...
					int step = (relativeFrame>127?127:relativeFrame);
					int waitTime = step & 0xFF;
					int frameDrop = (nbBytesSinceLastWait+1)/64;
					nbBytesSinceLastWait = 0;
					if (frameDrop > waitTime) {
						log.info("*********** WARNING *********** Timing does not match data speed. Deviation: +"+(frameDrop-waitTime)*20+" ms.");
					} else {
						if (frameDrop != waitTime) {
							data[dataIdx++] = (byte) (waitTime-frameDrop);
							log.debug("wait "+String.format("%02X", (data[dataIdx-1])));
							log.debug(" (Tick "+absolTick+")\n");
						}
					}
					relativeFrame -= step;
				}
				...
				for (int j = 0; j < message.getLength(); j++) {
					...
					log.debug(" "+String.format("%02X", (data[dataIdx-1])));
					nbBytesSinceLastWait++;
				}	
				
				log.debug("");
			} else {
				log.info("*********** SKIPPED *********** " + Arrays.toString(message.getMessage()) + " " + message.getClass().getSimpleName());
			}
		}
