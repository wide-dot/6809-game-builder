...
public class PageSetPlugin {
	...
	private static String[] unit(ImmutableNode node, BuildContext ctx, String gendir)
		...
		// A block holds what a unit holds — its code AND its images. It used
		// to take <asm> only, which meant an object could enter a pageset
		// without its sprites : the very thing that made the tail of a set
		// unusable for anything but a table. Same dispatch as <lwasm> uses, so
		// <gfxcomp>, <imageset>, <tilemap>, <animation> all work here.
		//
		// Nested containers get their own defaults and defines, as everywhere.
