...
25. [AABB screen projection: the whole box stays inside the
    byte](aabb-screen-projection.md) — the kernel compares centres modulo
    256 ; a v1 site writes the low byte raw, a v2 swept box is built from
    its clamped edges (`AABB.spanX`). Converted progressively, site by site.
